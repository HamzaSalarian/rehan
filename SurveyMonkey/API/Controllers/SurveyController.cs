...
namespace API.Controllers
{
    public class SurveyController : ApiController
    {
        ...
        public HttpResponseMessage surveyByApproved(int ap, string Discipline, string Section, string Semester_no, string status, string sex)
        {
            try
            {
                ...
                var rs = db.surveys.Join(db.ActiveSurveys, s => s.id, a => a.sid,
                    (s, a) => new { s, a })
                    .Where(w => w.a.discipline == Discipline && w.a.section == Section && w.a.semester == Semester_no && w.s.approved == 3)
                    .ToList();

                rs.ForEach(i => list.Add(i.s));

                var q = db.surveys.Where(i => i.approved == ap && i.status == status).ToList();

                q.ForEach(i =>
                {
                    if (i.sex == "B" || i.sex == sex)
                    {
                        list.Add(i);
                    }
                });
        ...
        [HttpGet]
        public HttpResponseMessage calculateGraph(int sid, string gender = "All", string discipline = "All")
        {
            try
            {
                var questions = db.surveyquestions.Where(i => i.surveyid == sid).ToList();

                var responsesQuery = db.surveyresponses.Where(i => i.surveyid == sid);

                if (gender != "All")
                {
                    responsesQuery = responsesQuery.Where(r => db.STMTRs.Any(st => st.Reg_No == r.userid && st.Sex == gender));
                }
                if (discipline != "All")
                {
                    responsesQuery = responsesQuery.Where(r => db.STMTRs.Any(st => st.Reg_No == r.userid && st.Discipline == discipline));
                }
                ...
                var optionPercentages = optionCounts.Select(count => totalCount > 0 ? (count * 100.0) / totalCount : 0).ToArray();
        ...
        public HttpResponseMessage getSurveyHistory()
        {
            try
            {
                ...
                var q1 = db.surveys.Join(db.EMPMTRs, s => s.createdby, e => e.Emp_no, (s, e) => new { s, e }).ToList();
                var q2 = db.surveys.Join(db.STMTRs, s => s.createdby, e => e.Reg_No, (s, e) => new { s, e }).ToList();
        ...
        [HttpGet]
        public HttpResponseMessage calculateTeacherGraph(int sid, string gender = "All", string discipline = "All")
        {
            try
            {
                ...
                questions.ForEach(q =>
                {
                    ...
                    list.Add(new TeacherGraph
                    {
                        question = q.title,
                        excelent = totalResponses > 0 ? ((double)e / totalResponses) * 100 : 0,
                        good = totalResponses > 0 ? ((double)g / totalResponses) * 100 : 0,
                        bad = totalResponses > 0 ? ((double)b / totalResponses) * 100 : 0,
                        worse = totalResponses > 0 ? ((double)w / totalResponses) * 100 : 0
                    });
                });
        ...
        public HttpResponseMessage task()
        {
            try
            {
                ...
                {
                    Students = db.STMTRs.Select(i => i).ToList(),
                    Surveys = db.surveys.Select(i => i).ToList()
                };
